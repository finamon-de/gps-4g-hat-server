import { BadRequestException, Body, Controller, Delete, Get, HttpStatus, NotFoundException, Param, Post, Query, Res } from '@nestjs/common';
...
import { ApiTags, ApiOperation, ApiBadRequestResponse, ApiNotFoundResponse, ApiParam, ApiQuery, ApiOkResponse } from "@nestjs/swagger";
...
export class AccSensorController {
    ...
    @ApiOperation({
        ...
        description: 'Add new sensor data. It is required that the `device` property is set.'
    })
    @ApiOkResponse({ description: "Success message" })
    @ApiBadRequestResponse({ description: "The `device` property wasn't set properly." })
    @Post("/")
    async addAccSensorData(
        @Res() res,
        @Body() createAccSensorDto: CreateAccSensorDTO
    ) {
        if (!createAccSensorDto.device || !Types.ObjectId.isValid(createAccSensorDto.device as unknown as Types.ObjectId)) {
    ...
    @ApiOkResponse({ description: "Success message" })
    @ApiNotFoundResponse({ description: "Sensor data record was not found or does not exist." })
    @Get("/:accSensorId")
