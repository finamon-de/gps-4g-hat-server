...
export class AccSensorService {
    constructor(@InjectModel("AccSensor") private readonly accSensorModel: Model<AccSensor>) {}

    async addAccSensorData(createAccSensorDto: CreateAccSensorDTO): Promise<AccSensor> {
    ...
    async getAccSensorDataObject(accSenosrId): Promise<AccSensor> {
        const accSensorDataObject = await this.accSensorModel.findById(accSenosrId).exec();
    ...
    async deleteAccSensorData(accSenosrId): Promise<AccSensor> {
        const deletedAccSensorData = await this.accSensorModel.findOneAndRemove(accSenosrId).exec();
