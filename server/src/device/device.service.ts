...
export class DeviceService {
    ...
    async editDevice(deviceId, createDeviceDto: CreateDeviceDTO): Promise<Device> {
        const editedDevice = await this.deviceModel.findByIdAndUpdate(deviceId, createDeviceDto, { new: true }).exec();
        return editedDevice;
    }
    ...
    async deleteDevice(deviceId): Promise<any> {
        const deletedDevice = await this.deviceModel.findByIdAndRemove(deviceId).exec();
